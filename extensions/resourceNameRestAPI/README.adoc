...
The project **resourceNameRestAPI** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `groovy` language.
...
The build should produce a zip archive under the `target/` folder named `resourceNameRestAPI-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `resourceNameRestAPI-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
